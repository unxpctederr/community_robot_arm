...
//STEPPER OBJECTS
RampsStepper stepperHigher(X_STEP_PIN, X_DIR_PIN, X_ENABLE_PIN, INVERSE_X_STEPPER, MAIN_GEAR_TEETH, MOTOR_GEAR_TEETH, MICROSTEPS, STEPS_PER_REV, 'X');
RampsStepper stepperLower(Y_STEP_PIN, Y_DIR_PIN, Y_ENABLE_PIN, INVERSE_Y_STEPPER, MAIN_GEAR_TEETH, MOTOR_GEAR_TEETH, MICROSTEPS, STEPS_PER_REV, 'Y');
RampsStepper stepperRotate(Z_STEP_PIN, Z_DIR_PIN, Z_ENABLE_PIN, INVERSE_Z_STEPPER, MAIN_GEAR_TEETH, MOTOR_GEAR_TEETH, MICROSTEPS, STEPS_PER_REV, 'Z');

//RAIL OBJECTS
#if RAIL
  RampsStepper stepperRail(E0_STEP_PIN, E0_DIR_PIN, E0_ENABLE_PIN, INVERSE_E0_STEPPER, MAIN_GEAR_TEETH, MOTOR_GEAR_TEETH, MICROSTEPS, STEPS_PER_REV, 'E');
...
void homeSequence(){
  ...
  if (HOME_Y_STEPPER && HOME_X_STEPPER){
    Logger::logINFO("HOMING Y AXIS (Lower Shank)");
    endstopY.home(!INVERSE_Y_STEPPER);
    Logger::logINFO("HOMING Y AXIS COMPLETE");

    Logger::logINFO("HOMING X AXIS (Upper Shank)");
    endstopX.home(!INVERSE_X_STEPPER);
    Logger::logINFO("HOMING X AXIS COMPLETE");
  } else {
  ...
  if (HOME_Z_STEPPER){
    Logger::logINFO("HOMING Z AXIS (Rotation)");
    endstopZ.home(INVERSE_Z_STEPPER);
    Logger::logINFO("HOMING Z AXIS COMPLETE");
  }
  #if RAIL
    if (HOME_E0_STEPPER){
